 ...
 *
 * G2PEngine.cpp - Grapheme-to-Phoneme conversion engine implementation
 *
...
std::vector<PhonemeResult> G2PEngine::applyRulesToWord(
    ...
    while (pos < word_lower.size()) {
        ...
        for (const auto& rule : all_rules) {
            ...
            // Check pattern match (simple literal match for now)
            // TODO: Support regex patterns
            if (substr != rule.pattern) {
                continue;
            }
